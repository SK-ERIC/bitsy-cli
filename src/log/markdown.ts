...
function formatReferences(
  ...
  const refs = references
    ...
    .map((ref) => {
      ...
      return `[<samp>(${ref.value.slice(
        0,
        5
      )})</samp>](https://github.com/${githubRepo}/commit/${ref.value})`;
    });
...
function formatLine(commit: GitCommit, options: ChangelogOption) {
  ...
  let authors = join([
    ...new Set(
      commit.resolvedAuthors.map((i) =>
        i.login ? `@${i.login}` : `**${i.name}**`
      )
    ),
  ]).trim();
  ...
  const description = options.capitalize
    ...
    .forEach((scope) => {
      ...
      lines.push(
        ...scopes[scope]
          .reverse()
          .map(
            (commit) => `${padding}- ${prefix}${formatLine(commit, options)}`
          )
      );
    });
...
export function generateMarkdown(params: {
  ...
  if (showTitle) {
    const date = isNewVersion
      ? dayjs().format("YY-MM-DD")
      : options.tagDateMap.get(options.to);
...
export async function writeMarkdown(
  ...
  if (lastEntry) {
    changelogMD = `${
      changelogMD.slice(0, lastEntry.index) + md
