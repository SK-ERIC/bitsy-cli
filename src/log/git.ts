...
async function getResolvedAuthorLogin(
  github: GithubConfig,
  commitHashes: string[],
  email: string
) {
  ...
  if (commitHashes.length) {
    try {
      const data = await ofetch(
        `https://api.github.com/repos/${repo}/commits/${commitHashes[0]}`,
        {
    ...
    for await (const [index, author] of commit.authors.entries()) {
      ...
      if (email && name) {
        ...
        if (!resolvedLogins?.has(email)) {
          const login = await getResolvedAuthorLogin(
            github,
            commitHashes,
            email
          );
